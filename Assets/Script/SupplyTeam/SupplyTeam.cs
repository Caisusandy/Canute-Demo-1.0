...
{
    public static class SupplyTeam
    {
        ...
        public static void GoOut(ExplorationTeam team)
        {
            List<PrizeBox> boxes = new List<PrizeBox>();
            ConditionalPrize prize;
            ...
            for (int i = 0; i < (count > 4 ? 4 : count); i++)
            {
                ...
                {
                    //story & letter
                    prize = StoryAndLetterPrize(team);
                    boxes.Add(new PrizeBox(prize));
                ...
                else if (range < 0.5)
                {
                    ...
                    WeightItem epic = new WeightItem((int)(1500 + 1000 * v), "epic");
                    WeightItem legnedary = new WeightItem((int)(500 + 500 * v), "legendary");
        ...
        private static ConditionalPrize StoryAndLetterPrize(ExplorationTeam team)
        {
            var i = 0;
            Story:
            while (true)
        ...
        private static ConditionalPrize GetPrize(ExplorationTeam team, Rarity rarity)
        {
            Debug.Log(rarity);
            Re:
            var fund = new Fund(100, 1000, 500);
...
namespace Canute.SupplyTeam
{
    ...
    public class ExplorationTeam
    {
        ...
        public bool IsOut => ToBackTime > TimeSpan.Zero;
