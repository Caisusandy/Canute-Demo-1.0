    ...
    public class CentralDeck : DataList<Card>
    {
        public void Generated()
        {
            for (int i = 1; i <= 4; i++)
            {
                ...
                for (int j = 0; j < 12; j++)
                {
                    Add(new Card(Card.Types.normal, career, new Effect(Effect.Types.enterAttack, 1), 2, TargetType.self | TargetType.armyEntity));
                }
                for (int j = 0; j < 12; j++)
                {
                    Add(new Card(Card.Types.normal, career, new Effect(Effect.Types.enterMove, 1), 3, TargetType.self | TargetType.armyEntity));
                }
            }
            for (int j = 0; j < 8; j++)
            {
